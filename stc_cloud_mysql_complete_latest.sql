...
DROP TABLE IF EXISTS `ai_alert_policies`;
DROP TABLE IF EXISTS `ai_camera_bindings`;
DROP TABLE IF EXISTS `ai_scenario_rules`;
DROP TABLE IF EXISTS `ai_scenarios`;
DROP TABLE IF EXISTS `free_trial_requests`;
DROP TABLE IF EXISTS `edge_nonces`;
...
CREATE TABLE `organization_subscriptions` (
    ...
    INDEX `idx_org_subscriptions_org_status` (`organization_id`, `status`),
    INDEX `idx_org_subscriptions_plan` (`subscription_plan_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- ============================================
-- 26. EDGE NONCES (HMAC Replay Protection)
-- ============================================
CREATE TABLE `edge_nonces` (
    `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
    ...
    `updated_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (`edge_server_id`) REFERENCES `edge_servers`(`id`) ON DELETE SET NULL,
    INDEX `idx_edge_nonces_nonce` (`nonce`),
    INDEX `idx_edge_nonces_server_used` (`edge_server_id`, `used_at`),
    INDEX `idx_edge_nonces_used_at` (`used_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- ============================================
-- 27. AI SCENARIOS (Enterprise Monitoring)
-- ============================================
CREATE TABLE `ai_scenarios` (
    ...
    `severity_threshold` INT DEFAULT 70,
    `config` JSON NULL,
    `created_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at` TIMESTAMP NULL,
    FOREIGN KEY (`organization_id`) REFERENCES `organizations`(`id`) ON DELETE CASCADE,
    INDEX `idx_ai_scenarios_org_module` (`organization_id`, `module`),
    INDEX `idx_ai_scenarios_org_enabled` (`organization_id`, `enabled`),
    UNIQUE KEY `unique_org_module_scenario` (`organization_id`, `module`, `scenario_type`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- ============================================
-- 28. AI SCENARIO RULES
-- ============================================
CREATE TABLE `ai_scenario_rules` (
    `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
    ...
    `created_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (`scenario_id`) REFERENCES `ai_scenarios`(`id`) ON DELETE CASCADE,
    INDEX `idx_ai_scenario_rules_scenario_enabled` (`scenario_id`, `enabled`),
    INDEX `idx_ai_scenario_rules_scenario_order` (`scenario_id`, `order`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- ============================================
-- 29. AI CAMERA BINDINGS
-- ============================================
CREATE TABLE `ai_camera_bindings` (
    `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
    `camera_id` BIGINT UNSIGNED NOT NULL,
    `scenario_id` BIGINT UNSIGNED NOT NULL,
    `enabled` BOOLEAN DEFAULT TRUE,
    `camera_specific_config` JSON NULL,
    `created_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (`camera_id`) REFERENCES `cameras`(`id`) ON DELETE CASCADE,
    FOREIGN KEY (`scenario_id`) REFERENCES `ai_scenarios`(`id`) ON DELETE CASCADE,
    UNIQUE KEY `unique_camera_scenario` (`camera_id`, `scenario_id`),
    INDEX `idx_ai_camera_bindings_camera_enabled` (`camera_id`, `enabled`),
    INDEX `idx_ai_camera_bindings_scenario_enabled` (`scenario_id`, `enabled`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- ============================================
-- 30. AI ALERT POLICIES
-- ============================================
CREATE TABLE `ai_alert_policies` (
    `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
    `organization_id` BIGINT UNSIGNED NOT NULL,
    ...
    `updated_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (`organization_id`) REFERENCES `organizations`(`id`) ON DELETE CASCADE,
    UNIQUE KEY `unique_org_risk_level` (`organization_id`, `risk_level`),
    INDEX `idx_ai_alert_policies_org_risk` (`organization_id`, `risk_level`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- ============================================
-- 31. FREE TRIAL REQUESTS (Sales Pipeline)
-- ============================================
CREATE TABLE `free_trial_requests` (
    ...
    `converted_organization_id` BIGINT UNSIGNED NULL,
    ...
    FOREIGN KEY (`converted_organization_id`) REFERENCES `organizations`(`id`) ON DELETE SET NULL,
    INDEX `idx_free_trial_requests_status` (`status`),
    INDEX `idx_free_trial_requests_email` (`email`),
    INDEX `idx_free_trial_requests_created_at` (`created_at`),
    INDEX `idx_free_trial_requests_assigned_admin` (`assigned_admin_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- ============================================
-- UPDATE EVENTS TABLE (Add Analytics Fields)
-- ============================================
ALTER TABLE `events` 
    ADD COLUMN IF NOT EXISTS `ai_module` VARCHAR(50) NULL AFTER `event_type`,
    ADD COLUMN IF NOT EXISTS `risk_score` INT NULL AFTER `severity`,
    ADD COLUMN IF NOT EXISTS `title` VARCHAR(255) NULL AFTER `severity`,
    ADD COLUMN IF NOT EXISTS `description` TEXT NULL AFTER `title`,
    ADD COLUMN IF NOT EXISTS `camera_id` VARCHAR(255) NULL AFTER `edge_id`,
    ADD COLUMN IF NOT EXISTS `acknowledged_at` TIMESTAMP NULL AFTER `meta`,
    ADD COLUMN IF NOT EXISTS `resolved_at` TIMESTAMP NULL AFTER `acknowledged_at`;

-- Add indexes for analytics
ALTER TABLE `events`
    ADD INDEX IF NOT EXISTS `idx_events_ai_module` (`ai_module`),
    ADD INDEX IF NOT EXISTS `idx_events_risk_score` (`risk_score`),
    ADD INDEX IF NOT EXISTS `idx_events_camera_id` (`camera_id`),
    ADD INDEX IF NOT EXISTS `idx_events_org_module_date` (`organization_id`, `ai_module`, `occurred_at`);

-- ============================================
-- UPDATE EDGE SERVERS TABLE (Add Security Fields)
-- ============================================
ALTER TABLE `edge_servers`
    ADD COLUMN IF NOT EXISTS `edge_key` VARCHAR(255) UNIQUE NULL AFTER `edge_id`,
    ADD COLUMN IF NOT EXISTS `edge_secret` VARCHAR(255) NULL AFTER `edge_key`,
    ADD COLUMN IF NOT EXISTS `secret_delivered_at` TIMESTAMP NULL AFTER `edge_secret`,
    ADD COLUMN IF NOT EXISTS `internal_ip` VARCHAR(255) NULL AFTER `ip_address`,
    ADD COLUMN IF NOT EXISTS `public_ip` VARCHAR(255) NULL AFTER `internal_ip`,
    ADD COLUMN IF NOT EXISTS `hostname` VARCHAR(255) NULL AFTER `public_ip`;

-- Add indexes
ALTER TABLE `edge_servers`
    ADD INDEX IF NOT EXISTS `idx_edge_servers_edge_key` (`edge_key`);

-- ============================================
-- UPDATE SUBSCRIPTION PLANS TABLE
-- ============================================
ALTER TABLE `subscription_plans`
    ADD COLUMN IF NOT EXISTS `sms_quota` INT UNSIGNED DEFAULT 0 AFTER `price_yearly`,
    ADD COLUMN IF NOT EXISTS `retention_days` INT UNSIGNED NULL AFTER `sms_quota`;

-- ============================================
-- UPDATE UPDATES TABLE (Add Versioning Fields)
-- ============================================
ALTER TABLE `updates`
    ADD COLUMN IF NOT EXISTS `version` VARCHAR(50) NULL AFTER `title`,
    ADD COLUMN IF NOT EXISTS `version_type` ENUM('major', 'minor', 'patch', 'hotfix') NULL AFTER `version`,
    ADD COLUMN IF NOT EXISTS `release_notes` TEXT NULL AFTER `body`,
    ADD COLUMN IF NOT EXISTS `changelog` TEXT NULL,
    ADD COLUMN IF NOT EXISTS `affected_modules` JSON NULL,
    ADD COLUMN IF NOT EXISTS `requires_manual_update` BOOLEAN DEFAULT FALSE,
    ADD COLUMN IF NOT EXISTS `download_url` VARCHAR(500) NULL,
    ADD COLUMN IF NOT EXISTS `checksum` VARCHAR(255) NULL,
    ADD COLUMN IF NOT EXISTS `file_size_mb` INT NULL,
    ADD COLUMN IF NOT EXISTS `release_date` TIMESTAMP NULL,
    ADD COLUMN IF NOT EXISTS `end_of_support_date` TIMESTAMP NULL;

-- ============================================
-- UPDATE PLATFORM CONTENTS TABLE
-- ============================================
ALTER TABLE `platform_contents`
    ADD COLUMN IF NOT EXISTS `published` BOOLEAN DEFAULT FALSE AFTER `section`;

-- ============================================
-- REMOVE BIOMETRIC COLUMNS (Compliance)
-- ============================================
-- Note: face_encoding and plate_encoding are removed for compliance
-- These columns should NOT exist in the database
