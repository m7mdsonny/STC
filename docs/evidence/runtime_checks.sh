...
# Laravel app to be running locally on https://127.0.0.1:8443 and seeded data
# with at least one edge server and license belonging to the same organization.
...
curl -vk "http://127.0.0.1:8443/api/v1/ping" -o /tmp/http_response.txt -w "\nstatus:%{http_code}\n" 2>>"$HTTP_LOG" | tee -a "$HTTP_LOG"

printf "\n[HTTPS] request should succeed" | tee -a "$HTTP_LOG"
curl -sk "https://127.0.0.1:8443/api/v1/ping" -o /tmp/https_response.txt -w "\nstatus:%{http_code}\n" 2>>"$HTTP_LOG" | tee -a "$HTTP_LOG"
...
if [[ -z "$EDGE_KEY" || -z "$EDGE_SECRET" || -z "$LICENSE_KEY" ]]; then
  ...
  curl -sk -X POST "https://127.0.0.1:8443/api/license/validate" \
